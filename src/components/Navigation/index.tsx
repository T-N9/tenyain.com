    ...
    <nav className=" fixed w-full z-50 top-0 start-0 ">
      <div className="max-w-screen-lg rounded-b-xl flex flex-wrap items-center justify-between mx-auto py-2 px-4 xl:px-8 lg:mt-5 bg-white/50 dark:bg-black/30 backdrop-blur-[10px] transition-colors border dark:border-gray-700 lg:rounded-xl">
        ...
        <div className={`items-center justify-between ${isOpen ? 'block' : 'hidden'} w-full lg:flex lg:w-auto lg:order-1`} id="navbar-sticky">
          <ul className="flex flex-col p-4 lg:p-0 mt-4 font-medium border border-gray-100 rounded-lg lg:space-x-2 rtl:space-x-reverse lg:flex-row lg:mt-0 lg:border-0 dark:border-gray-700">
            {navItems.map((item) => (
