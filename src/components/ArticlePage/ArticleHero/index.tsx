...
const ArticleHero: React.FC<ArticleHeroProps> = ({ title, createdAt, tags, image }) => {
  ...
  return (
    <div className='flex flex-col gap-3'>
      <h1 className='text-3xl lg:text-5xl  font-bold text-primary-600 dark:text-accent-600'>{title}</h1>
      <div className='flex justify-between mt-1 lg:mt-5'>
        <p className='text-gray-500'>{moment(createdAt).format('MMMM Do, YYYY')}</p>
        <div className='flex gap-2'>
          {tags.map((tag) => (
              <Link key={tag} href={`/${locale}/writing/tag/${tag}`}>
                  <Badge  key={tag} size="sm" color='gray' className='border dark:border-slate-700 text-sm'>
                      {tag}
                  </Badge>
              </Link>
